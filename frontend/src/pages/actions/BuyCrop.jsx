...
const BuyCrop = () => {
  ...
  const fetchAvailableCrops = async () => {
    try {
      const response = await retailerAPI.getAvailableCrops();
  ...
  const handlePurchase = async (e) => {
    ...
    setLoading(true);
    try {
      await retailerAPI.buyCrop(selectedCrop.id, parseFloat(purchaseQuantity));
    ...
    <div className="action-page">
      ...
      >
        <div className="action-header">
          <h1>🏪 Buy Crops from Farmers</h1>
          <button onClick={() => navigate('/retailer-dashboard')} className="btn-back">
            ← Back to Dashboard
          </button>
        </div>
        ...
        {!selectedCrop ? (
          <div className="crops-grid">
            ...
            {crops.length === 0 ? (
              <p className="no-data">No crops available for purchase at the moment.</p>
            ) : (
              <div className="grid">
                {crops.map((crop) => (
                  <motion.div
